...
const Stars = ({ count = 5 }) => (
  <div className="flex justify-center gap-0.5 mb-4">
    {Array.from({ length: 5 }).map((_, i) => (
      <span key={i} className={i < count ? "text-black text-lg" : "text-gray-300 text-lg"}>★</span>
    ))}
  </div>
);
...
function HeroSlider() {
  ...
  return (
    ...
    >
      {/* decorative blobs */}
      <div className="absolute left-0 top-1/2 w-48 h-64 opacity-40 pointer-events-none"
        style={{ background: "radial-gradient(ellipse at 30% 50%, #c9898f 0%, transparent 70%)", filter: "blur(18px)", transform: "translateY(-50%) scaleX(0.6) rotate(-8deg)" }} />
      <div className="absolute right-0 top-1/4 w-40 h-48 opacity-30 pointer-events-none"
        style={{ background: "radial-gradient(ellipse at 70% 30%, #c9898f 0%, transparent 70%)", filter: "blur(22px)" }} />
      ...
      <div className="relative" style={{ minHeight: "340px" }}>
          ...
          >
            <div className="relative z-10 max-w-6xl mx-auto px-6 py-12 flex flex-col md:flex-row items-center gap-8 md:gap-16">
              ...
              <div className="flex-1 flex flex-col items-start">
                <span className="text-2xl leading-none select-none mb-2">✦</span>
                <h1
                ...
                <p className="text-lg md:text-xl font-black text-gray-800 mb-2" style={{ letterSpacing: "0.22em" }}>
                  {s.sub}
                </p>
              </div>
              ...
              <div className="relative flex-1 flex justify-center items-end">
                ...
                <div className="relative bg-white p-2 shadow-lg" style={{ maxWidth: "420px", width: "100%" }}>
                  ...
                  <a
                    href={s.shopHref}
                    className="absolute -bottom-5 left-1/2 -translate-x-1/2 bg-black text-white text-sm font-black px-12 py-3 hover:bg-gray-800 transition-colors whitespace-nowrap"
        ...
        <div className="invisible pointer-events-none">
          <div className="max-w-6xl mx-auto px-6 py-12 flex flex-col md:flex-row items-center gap-8 md:gap-16">
            <div className="flex-1">
              <div className="text-7xl md:text-8xl leading-none mb-1">&nbsp;</div>
              <div className="text-5xl mb-4">&nbsp;</div>
              <div className="text-lg mb-2">&nbsp;</div>
            </div>
            <div className="flex-1">
              <div style={{ maxWidth: "420px", width: "100%", aspectRatio: "4/3" }} />
            </div>
          </div>
        </div>
      </div>
      ...
      {/* controls */}
      <div className="relative z-10 flex items-center justify-center gap-4 pb-8 mt-4">
        <button onClick={() => go(current - 1)} aria-label="Previous slide" className="text-gray-500 hover:text-black transition-colors p-1">
        ...
        <button onClick={() => go(current + 1)} aria-label="Next slide" className="text-gray-500 hover:text-black transition-colors p-1">
          <ChevronRight size={18} strokeWidth={1.5} />
        </button>
        <button onClick={() => setPlaying((v) => !v)} aria-label={playing ? "Pause" : "Play"} className="text-gray-500 hover:text-black transition-colors p-1 ml-1">
    ...
    <section className="py-14 px-6" style={{ background: "#fce8e8" }}>
      <Reveal>
        <div className="text-center mb-10">
          <h2 className="text-2xl font-black text-gray-900 mb-3" style={{ letterSpacing: "0.18em" }}>
            SHOP OUR PICKS!
          </h2>
      ...
      <div className="max-w-5xl mx-auto bg-white p-6 grid grid-cols-1 sm:grid-cols-3 gap-0">
        {PRODUCTS.map((p, i) => (
          <Reveal key={p.id} delay={i * 120}>
            <a
              href={p.href}
              className={`group block relative ${i < PRODUCTS.length - 1 ? "sm:border-r border-gray-100" : ""}`}
            >
              {p.isSale && (
                <span className="absolute top-3 left-3 z-10 text-white text-xs font-bold px-2 py-0.5 rounded-full" style={{ background: "#d4a0a0" }}>
                  Sale
                </span>
              )}
              <div className="overflow-hidden" style={{ aspectRatio: "1/1" }}>
                <img src={p.img} alt={p.imgAlt} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
              </div>
              <div className="pt-4 pb-6 px-2 text-center">
                <h3 className="text-xs font-bold mb-2" style={{ color: "#d4a0a0", letterSpacing: "0.14em" }}>{p.name}</h3>
                <div className="flex items-center justify-center gap-2 text-sm font-semibold text-gray-800">
                  {p.originalPrice && <span className="line-through text-gray-400 text-xs">${p.originalPrice.toFixed(2)} USD</span>}
...
function Subscribe() {
  ...
  return (
    <section className="relative overflow-hidden">
      {/* White top — "subscribe" watermark fills it, envelope sits at the very bottom edge */}
      <div className="relative bg-white pt-16 pb-0 flex flex-col items-center">
        <Reveal>
          <span
            className="absolute inset-0 flex items-center justify-center text-[10vw] font-black select-none pointer-events-none"
            style={{ fontFamily: "Georgia, 'Times New Roman', serif", fontStyle: "italic", color: "#f0d0d0", opacity: 0.6, whiteSpace: "nowrap", lineHeight: 1 }}
          >
            subscribe
          </span>
        </Reveal>

        {/* Envelope pushed to bottom, half-overlapping the pink section */}
        <Reveal delay={100}>
          <div className="relative z-10 mb-0" style={{ transform: "translateY(50%)" }}>
        ...
        {/* spacer so white section has enough height */}
        <div className="h-16" />
      </div>

      {/* Pink bottom — content starts below the overlapping envelope */}
      <div className="relative flex flex-col items-center px-6 pt-16 pb-14" style={{ background: "linear-gradient(to bottom, #fce8e8 0%, #f5d5d5 100%)" }}>
        <Reveal delay={200}>
          <h2 className="text-lg md:text-xl font-black text-gray-900 text-center mb-4" style={{ letterSpacing: "0.18em" }}>
        ...
        <Reveal delay={300}>
          ...
          {submitted ? (
            <p className="text-sm font-semibold text-gray-700 tracking-wider">Thanks for subscribing! 🎉</p>
          ) : (
      ...
      {/* Wavy divider blending into Kind Words section */}
      <div className="relative" style={{ background: "linear-gradient(to bottom, #f5d5d5 0%, #e8c4c4 100%)", height: "120px" }}>
        <svg viewBox="0 0 1440 120" preserveAspectRatio="none" className="absolute top-0 left-0 w-full h-full" style={{ transform: "translateY(-1px)" }}>
          <path d="M0,40 C240,80 360,20 600,50 C840,80 1080,30 1320,60 L1440,70 L1440,0 L0,0 Z" fill="#f5d5d5" />
        </svg>
      </div>
    </section>
  );
...
function KindWords() {
  return (
    <section className="relative px-6 overflow-hidden" style={{ background: "#e8c4c4", paddingBottom: "64px" }}>
        ...
        <div className="relative z-10 flex flex-col items-center mb-12">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="bg-white px-8 py-4 shadow" style={{ lineHeight: 1 }}>
              <h2 className="text-4xl font-black tracking-wide text-gray-900" style={{ fontFamily: "'Arial Black', sans-serif", letterSpacing: "0.06em" }}>
                KIND WORDS
              </h2>
            </div>
          </div>
          <p className="mt-4 text-xs font-bold text-gray-700" style={{ letterSpacing: "0.32em" }}>FROM OUR CUSTOMERS</p>
        </div>
      </Reveal>

      <div className="relative z-10 max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
        {REVIEWS.map((r, i) => (
          <Reveal key={r.id} delay={i * 150}>
            <div className="flex flex-col items-center">
              <Stars count={r.stars} />
              <div className="bg-white p-6 w-full text-center shadow-sm">
                <h3 className="text-xs font-bold mb-4" style={{ color: "#d4a0a0", letterSpacing: "0.16em" }}>{r.title}</h3>
                <p className="text-sm text-gray-700 leading-relaxed mb-6">{r.body}</p>
                <p className="text-xs font-bold text-gray-500" style={{ letterSpacing: "0.18em" }}>{r.author}</p>
              </div>
            </div>
          </Reveal>
        ))}
